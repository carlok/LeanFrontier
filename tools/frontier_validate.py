...
def validate_metadata(record: Any, submission_id: str, limits: dict[str, Any], report: Report, path: str) -> None:
    ...
    if record.get("base_mathlib_revision") != "v4.33.0-rc1":
...
def static_preflight(base: Path | None, candidate: Path, limits: dict[str, Any], report: Report) -> tuple[dict[str, Path | None], dict[str, Any] | None]:
    ...
    for relative, path in changed.items():
        ...
        if match:
            ...
            try:
                ...
                validate_metadata(parsed, match.group(1), limits, report, relative)
...
def mathlib_duplicates(candidate: Path, hints: set[str], limits: dict[str, Any], report: Report) -> set[str] | None:
    """Ask Lean to scan Mathlib, emitting only candidate hash-prefilter matches.

    Each returned canonical expression is then SHA-256 hashed in Python, so a
    non-cryptographic Lean hash collision can never cause a rejection.
    """
    try:
        if not hints:
            return set()
        match_arg = "--match=" + ",".join(sorted(hints))
        process = run(["lake", "exe", "frontier-audit", "--", "--all", match_arg, "Mathlib"], candidate, limits["validation_timeout_seconds"])
        if process.returncode:
            report.reject("BUILD_FAILED", "could not compare candidate fingerprints with pinned Mathlib")
            return None
        matches = {digest for item in parse_audit(process.stdout).values() if item.get("kind") == "theorem" for digest in [canonical_digest(item)] if digest is not None}
...
def lean_audit(candidate: Path, modules: list[str], metadata: dict[str, Any], limits: dict[str, Any], axioms: dict[str, Any], report: Report) -> None:
    ...
    baseline_fingerprints = mathlib_duplicates(candidate, candidate_hints, limits, report)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        changed, metadata = static_preflight(base, candidate, limits, report)
